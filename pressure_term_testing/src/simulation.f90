...
contains
   ...
   function Calc_RMS_Velocity() result(VrmsGlobal)
      ...
      integer :: i,j,k
      integer :: ierr
      real(WP) :: Vrms,Umag,count,VrmsGlobal,countGlobal,dx,dy
      ! Flow variables
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         call param_Read('Smoothing Option',SmoothingOption)
      ...
      create_monitor: block
         ...
         call mfile%add_column(Tv,'Viscous Time Scale')
         call mfile%add_column(Tc,'Capillary Time Scale')
   ...
   ! subroutine applyGradientSmoothing(fs,vf) OLD VERSION ON Gradient Values
   !    use irl_fortran_interface
   !    use f_PUNeigh_RectCub_class
   !    use f_SeparatorVariant_class
   !    use f_PUSolve_RectCub_class
   ...
   !       do j=cfg%jmin_,cfg%jmax_
   !          do i=cfg%imin_,cfg%imax_
   !             tot_grad_x = SUM(grad_vf_x(i-1:i+1,j-1:j+1,k))
   !             tot_grad_y = SUM(grad_vf_y(i-1:i+1,j-1:j+1,k))

   !             tot_grad_vf_x(i,j,k) = tot_grad_x 
   !             tot_grad_vf_y(i,j,k) = tot_grad_y
   !             ! X Direction
   !             if(grad_vf_x(i,j,k) .gt. 1e-12) then  
   !                write(*,'(A,3I10.5)') ' ==== location: ', i,j,k 
   !                do ii = -1,1
   !                   do jj = -1,1
   !                      do kk = 0,0
   !                         Fst_x(i,j,k) = Fst_x(i,j,k) + grad_vf_x(i+ii,j+jj,k+kk)*fs%Pjx(i+ii,j+jj,k+kk) ! Summing Components
   !                         write(*,'(A,F10.5)') ' ==== grad_vf_x: ', grad_vf_x(i+ii,j+jj,k+kk)
   !                         write(*,'(A,F10.5)') ' ==== Pjx: ', fs%Pjx(i+ii,j+jj,k+kk)
   !                      enddo
   !                   enddo
   !                enddo
   !                write(*,'(A,F10.5)') ' ==== tot_grad_vf_x: ', tot_grad_vf_x(i,j,k)

   !                Fst_x(i,j,k) = Fst_x(i,j,k)/tot_grad_vf_x(i,j,k) ! Divide by total of weights to keep force "the same"
   !                write(*,'(A,F10.5)') ' ====  Fst_x(i,j,k): ', Fst_x(i,j,k)
   !             endif 
