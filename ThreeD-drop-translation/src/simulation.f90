...
module simulation
   ...
   real(WP) :: Tbot,Ttop,Ly,Lx,dtps,RootMeanSquareVelocity
   logical :: twoD
...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         use mathtools, only: Pi
         integer :: i,j,k,n,si,sj,sk,reconType
         ...
         real(WP) :: B
         ! Create a VOF solver
         call param_read('Reconstruction Type',reconType)
      ...
      create_and_initialize_flow_solver: block
         ...
         real(WP) :: Re,La,U,delta,dtCap,minTc,minTv,nx,time_scale_factor,Tv,Tc
         ...
         ! initial field
         call param_read('Droplet Velocity',U1)
         U1 = (/U,0.0_WP,0.0_WP/)
         fs%U = U1(1)
         fs%V = U1(2)
         fs%W = U1(3)
         ...
         fs%visc_l = sqrt(fs%rho_l*fs%sigma*2*radius/La)
         fs%visc_g = sqrt(fs%rho_l*fs%sigma*2*radius/La)
      ...
      create_monitor: block
         ...
         call mfile%add_column(RootMeanSquareVelocity,'RMS Velocity')
         call mfile%write()
         ! Create CFL monitor
