               ...
               do j=vf%cfg%jmin_,vf%cfg%jmax_
                  do i=vf%cfg%imin_,vf%cfg%imax_
                     if(vf%VF(i,j,k) .gt. 1e-12 .and. vf%VF(i,j,k) .lt. 1.0_WP - 1e-12) then 
                        ...
                        ! Compute Direction
                        if(abs(planeNormal(1)) .gt. abs(planeNormal(2)) .and. abs(planeNormal(1)) .gt. abs(planeNormal(3))) then ! Plane more vertical, X orient
                           dir = int(sign(1.0_WP,planeNormal(1)))
                        elseif(abs(planeNormal(2)) .gt. abs(planeNormal(1)) .and. abs(planeNormal(2)) .gt. abs(planeNormal(1))) then
                           dir = int(sign(2.0_WP,planeNormal(2)))
                        else 
                           dir = int(sign(3.0_WP,planeNormal(3)))
                        endif

                        call vf%height_function_curvature(i,j,k,dir,vf%curv(i,j,k))

                     endif
                  enddo
               enddo
            enddo
            call fs%cfg%sync(vf%curv)
         end block update_curvature
