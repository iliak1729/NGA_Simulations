...
contains
   ...
   function Calc_Center_of_mass_velocity() result(count)
      ...
      real(WP), dimension(:,:,:), allocatable :: VOF
      real(WP), dimension(:,:,:,:), allocatable :: LiquidBarycenter
      real(WP) :: count,VFTotal(1),VFTotalGlobal(1)
      real(WP), dimension(3) :: CenterOfMassV,CellCenterVelocity
      real(WP), dimension(3) :: CenterOfMassVGlobal
      real(WP) :: ULoc,VLoc,WLoc
      integer :: ierr
      ...
      ! MPI_ALLREDUCE OR MPI_REDUCE
      CALL MPI_ALLREDUCE(CenterOfMassV,CenterOfMassVGlobal,3,MPI_DOUBLE_PRECISION,MPI_SUM,cfg%comm,ierr)
      CALL MPI_ALLREDUCE(VFTotal,VFTotalGlobal,1,MPI_DOUBLE_PRECISION,MPI_SUM,cfg%comm,ierr)
      ! write(*,'(A,6F20.10)') "GLOBAL COMV,COMV: ", CenterOfMassVGlobal,CenterOfMassV
      CenterOfMassVGlobal = CenterOfMassVGlobal/max(abs(VFTotalGlobal(1)),1e-6) ! Reduce
      CenterOfMassV = CenterOfMassV/max(abs(VFTotal(1)),1e-6)
      ...
      UCOM = CenterOfMassVGlobal(1)
      VCOM = CenterOfMassVGlobal(2)
      WCOM = CenterOfMassVGlobal(3)
   ...
   subroutine simulation_run
      ...
      ! if (cfg%amRoot) write(*,'(A)') 'Start Sim'
      do while (.not.time%done())
         ...
         ! VOF solver step
         ! if (cfg%amRoot) write(*,'(A)') 'VF Advance'
         call vf%advance(dt=time%dt,U=fs%U,V=fs%V,W=fs%W)
         
         ! Apply Boundary Conditions
         ! if (cfg%amRoot) write(*,'(A)') 'VF BC'
         ...
         do while (time%it.le.time%itmax)
            ...
            ! if (cfg%amRoot) write(*,'(A)') 'FS Solve Implicit'
            ...
            ! if (cfg%amRoot) write(*,'(A)') 'FS Bcond'
            call fs%apply_bcond(time%t,time%dt)
            ! STOP
            ! Solve Poisson equation
            ! if (cfg%amRoot) write(*,'(A)') 'FS Laplacian'
            call fs%update_laplacian(pinpoint=[fs%cfg%imin,fs%cfg%jmin,fs%cfg%kmin])

            call fs%correct_mfr()
            call fs%get_div()
            ! call fs%add_surface_tension_jump(dt=time%dt,div=fs%div,vf=vf)
            ! if (cfg%amRoot) write(*,'(A)') 'FS Add Surface Tension'
            SELECT CASE (SurfaceTensionOption)
               CASE (2)
                  call add_surface_tension_jump_no_ST(fs,dt=time%dt,div=fs%div,vf=vf)
               CASE (3)
            ...
            fs%psolv%sol=0.0_WP
            ! if (cfg%amRoot) write(*,'(A)') 'FS Solve'
            call fs%psolv%solve()
