...
contains
   ...
   function Calc_Amplitude() result(heightGlobalRet)
      use mpi_f08, only: MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPI_MAX
      implicit none
      integer :: i,j,k,ierr,nx
      real(WP), dimension(:,:,:), allocatable :: VOF
      real(WP), dimension(:), allocatable :: tempLocal,tempGlobalArr

      real(WP) :: temp,heightGlobalRet
      real(WP), dimension(1) :: heightA,heightGlobalA
      real(WP), PARAMETER :: PI = 3.141592653589793
      ...
      nx = cfg%imax_ - cfg%imin_ +1
      allocate(tempLocal(nx),tempGlobalArr(nx))
      temp = 0.0
      VOF = vf%VF
      heightA(1) = 0.0
      heightGlobalA(1) = 0.0
      ...
      do i = cfg%imin_,cfg%imax_ ! Loop in x direction
         temp = 0.0
         do j =  cfg%jmin_,cfg%jmax_ ! Loop in y direction, finding height in column
            temp = temp + VOF(i,j,k)
         enddo
         tempLocal(i - cfg%imin_+1) = temp
         ! if(cfg%amRoot) write(*,'(A,F10.5)') "tempLocal: ",tempLocal(i - cfg%imin_+1)
      enddo
      
      ! Sum in Y direction to get column heights into tempGlobalArr
      call MPI_ALLREDUCE(tempLocal,tempGlobalArr,nx,MPI_DOUBLE_PRECISION,MPI_SUM,cfg%ycomm,ierr)

      do i = 1,nx
         if (tempGlobalArr(i) > heightA(1)) heightA(1) = tempGlobalArr(i)

         ! if(cfg%amRoot) write(*,'(A,F10.5)') "tempGlobalArr: ",tempGlobalArr(i)
      enddo

      ! write(*,'(A,F10.5)') "Height Found: ",heightA
      CALL MPI_ALLREDUCE(heightA,heightGlobalA,1,MPI_DOUBLE_PRECISION,MPI_MAX,cfg%xcomm,ierr)
      ! if(cfg%amRoot) write(*,'(A,F10.5)') "Height Found: ",heightA
      ! Now that we have maximum height, multiply by dy to get to dimensional terms
      heightGlobalA = heightGlobalA * cfg%dy(2)
      ! if(cfg%amRoot) write(*,'(A,F10.5)') "Height Dimensional: ",heightGlobalA
      ! Now subtract the half domain size to go to amplitude from 0 (assuming unit domain size)
      heightGlobalA = heightGlobalA - cfg%yL/2
      ! if(cfg%amRoot)  write(*,'(A,F10.5)') "Height Ret: ",heightGlobalA
      heightGlobalRet = heightGlobalA(1)
      ! STOP
      deallocate(tempLocal,tempGlobalArr)
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         ! Increment time
         call fs%get_cfl(time%dt,time%cfl)
         ! write(*,'(A)') '=================================== CFL Complete'
         call time%adjust_dt()
         call time%increment()
         ! write(*,'(A)') '=================================== Time Adjust Complete'
         ! Remember old VOF
         vf%VFold=vf%VF
         ...
         ! VOF solver step
         ! write(*,'(A)') '=================================== Advancing VF'
         call vf%advance(dt=time%dt,U=fs%U,V=fs%V,W=fs%W)
         ! write(*,'(A)') '=================================== Advance VF Complete'
         ...
         do while (time%it.le.time%itmax)
            ...
            SELECT CASE (SurfaceTensionOption)
               ...
               CASE DEFAULT
                  call fs%add_surface_tension_jump(dt=time%dt,div=fs%div,vf=vf)
            END SELECT
            !  write(*,'(A)') '=================================== Surface Tension Jump Complete'
            fs%psolv%rhs=-fs%cfg%vol*fs%div/time%dt
            if (cfg%amRoot) fs%psolv%rhs(cfg%imin,cfg%jmin,cfg%kmin)=0.0_WP
            fs%psolv%sol=0.0_WP
            call fs%psolv%solve()
            call fs%shift_p(fs%psolv%sol)
            ! write(*,'(A)') '=================================== Shift P Complete'
            ! Correct velocity
            call fs%get_pgrad(fs%psolv%sol,resU,resV,resW)
            ! write(*,'(A)') '=================================== Grad P Complete'
   ...
   subroutine simulation_final
      ...
      deallocate(resU,resV,resW,Ui,Vi,Wi,sigma_xx,sigma_yy,sigma_xy,sigma_yx,Fst_x,Fst_y,Fst_z,PjxD,PjyD,PjzD,SurfaceTensionDiv) 
         ...
         do j=PuCfg%jmin_,PuCfg%jmax_
            do i=PuCfg%imin_,PuCfg%imax_
               ...
               ! if(vf%VF(i,j,k) .gt. vf%VFmin .and. vf%VF(i,j,k) .lt. vf%VFmax) then  
                  ...
                  do j_in = -3,3
                     do i_in = -3,3
                        if(vf%VF(ind(1)+i_in,ind(2)+j_in,ind(3)) .gt. vf%VFmin .and. vf%VF(ind(1)+i_in,ind(2)+j_in,ind(3)) .lt. vf%VFmax) then
                  ...
                  ! Set Neighborhood in solver 
                  call setNeighborhood(solver,neighborhood)
                  if(ind(1) .eq. 4 .and. ind(2) .eq. 1 .and. count .lt. 1) then 
