...
module simulation
   ...
   integer :: ierr2, myrank,integerCatcher
...
contains
   ...
   function Calc_Amplitude() result(height)
      ...
      omega = sqrt(fs%sigma * (2*PI)*(2*PI)*(2*PI)/(2))
      omegaT = time%t*omega
   ...
   function Calc_Center_of_mass_velocity() result(count)
      implicit none
      integer :: i,j,k 
      real(WP), dimension(:,:,:), allocatable :: VOF
      real(WP), dimension(:,:,:,:), allocatable :: LiquidBarycenter
      real(WP) :: count,VFTotal
      real(WP), dimension(3) :: CenterOfMassV,CellCenterVelocity
      ...
      do i = cfg%imin,cfg%imax
         do j = cfg%jmin,cfg%jmax
            ...
            CenterOfMassV = CenterOfMassV + VOF(i,j,k) * CellCenterVelocity
            count = count + 1
         end do
      end do
      CenterOfMassV = CenterOfMassV/VFTotal

      UCOM = CenterOfMassV(1)
      VCOM = CenterOfMassV(2)
      WCOM = CenterOfMassV(3)
   end function Calc_Center_of_mass_velocity
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         ! For Solve Edge
         call param_read('Marangoni',MarangoniOption)
         call param_read('Pressure',PressureOption)
         ...
         VOF_Neumann : block
               ...
               ! write(*,'(A,3I25.5)') 'Current Properties Min',vf%cfg%imino_,vf%cfg%jmino_,vf%cfg%kmino_
               ! write(*,'(A,3I25.5)') 'Current Properties Max',vf%cfg%imaxo_,vf%cfg%jmaxo_,vf%cfg%kmaxo_
               do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                  do j = vf%cfg%jmino_,vf%cfg%jmaxo_
                     do i = vf%cfg%imino_,vf%cfg%imin-1 ! This makes it loop through only the left ghost cells (Indicies -2,-1,0)
                        ! Make VF
                        ! write(*,'(A,3I25.5)') 'Ghost Index',i,j,k
                        ! write(*,'(A,3I25.5)') 'Real Index',vf%cfg%imin-i,j,k
                        ! write(*,'(A)') '=================================== Get Vof'
               ...
               !          if(vf%curv(i,j,k) .ne. 0.0_WP) then
               !             ! write(*,'(A)')  '========= Eval Curvature'
               !             write(*,'(A,F25.10)')  '==================================== Regular Curvature: ',vf%curv(i,j,k)
               !             call heightFunctionCurvature(i,j,k,vf,vf%curv(i,j,k),dir)
      ...
      create_and_initialize_flow_solver: block
         ...
         ! call param_read('Laplace Number',laplace_number)
         ...
         SELECT CASE (BoundaryConditionOption)
            CASE (2)
               ...
               call fs%add_bcond(name='top',type=dirichlet,face='y',dir=1,canCorrect=.false.,locator=yp_locator)
         ...
         call param_read('Liquid Viscosity',fs%visc_l)
         call param_read('Gas Viscosity',fs%visc_g)
         ...
         fs%gravity = [0.0_WP,-9.81_WP,0.0_WP]
   ...
   subroutine updateSurfaceTensionStresses(fs,vf)
      ...
      ! write(*,'(A,3F20.5)') '=============== Marangoni: ',MarangoniOption
         ...
         do j=cfg%jmin_,cfg%jmax_
            do i=cfg%imin_,cfg%imax_
               ! if(vf%VF(i,j,k) .gt. vf%VFmin .and. vf%VF(i,j,k) .lt. vf%VFmax) then  
                  ...
                  ! write(*,'(A,3F10.5)') ' Marangoni Option :',MarangoniOption
                  ...
                  ! if(abs(force(1)) .gt. 1e-6 .or. abs(force(2)) .gt. 1e-6) then
                  !    STOP
                  ! endif
                  ! On the way, also fill in the boundary conditions? Not Now...  
               ! endif
            end do
         end do
      end do
      ...
      SELECT CASE (BoundaryConditionOption) 
         CASE (2)
               ...
               do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                  do j = vf%cfg%jmino_,vf%cfg%jmaxo_
                     do i = vf%cfg%imino_,vf%cfg%imin-1 ! This makes it loop through only the left ghost cells (Indicies -2,-1,0)
                        sigma_xx(i,j,k) = sigma_xx(vf%cfg%imin-i,j,k)
                        sigma_xy(i,j,k) = sigma_xy(vf%cfg%imin-i,j,k)
                        sigma_yx(i,j,k) = sigma_yx(vf%cfg%imin-i,j,k)
                        sigma_yy(i,j,k) = sigma_yy(vf%cfg%imin-i,j,k)
                     enddo
                  enddo
               enddo
            ...
            end block Stress_Neumann
            CASE (3)
               Stress_NeumannB : block
               integer :: i,j,k
               ...
               do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                  do j = vf%cfg%jmino_,vf%cfg%jmaxo_
                     do i = vf%cfg%imino_,vf%cfg%imin-1 ! This makes it loop through only the left ghost cells (Indicies -2,-1,0)
                        sigma_xx(i,j,k) = sigma_xx(vf%cfg%imax+i,j,k)
                        sigma_xy(i,j,k) = sigma_xy(vf%cfg%imax+i,j,k)
                        sigma_yx(i,j,k) = sigma_yx(vf%cfg%imax+i,j,k)
                        sigma_yy(i,j,k) = sigma_yy(vf%cfg%imax+i,j,k)
                     enddo
                  enddo
               enddo
               ...
               do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                  do j = vf%cfg%jmino_,vf%cfg%jmaxo_
                     do i = vf%cfg%imino_,vf%cfg%imin-1 ! This makes it loop through only the left ghost cells (Indicies -2,-1,0)
                        sigma_xx(vf%cfg%imax-i+1,j,k) = sigma_xx(vf%cfg%imin-i,j,k)
                        sigma_xy(vf%cfg%imax-i+1,j,k) = sigma_xy(vf%cfg%imin-i,j,k)
                        sigma_yx(vf%cfg%imax-i+1,j,k) = sigma_yx(vf%cfg%imin-i,j,k)
                        sigma_yy(vf%cfg%imax-i+1,j,k) = sigma_yy(vf%cfg%imin-i,j,k)
                     enddo
                  enddo
               enddo

            end block Stress_NeumannB

            CASE (4)
               Stress_NeumannC : block
               integer :: i,j,k
            ...
            end block Stress_NeumannC
            CASE DEFAULT
               ! Periodic Boundaries
               Stress_Fully_Periodic : block
                  integer :: i,j,k
                  ...
                  do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                     do j = vf%cfg%jmino_,vf%cfg%jmin-1
                        do i = vf%cfg%imino_,vf%cfg%imaxo_
                           sigma_xx(i,j,k) = sigma_xx(i,vf%cfg%jmax+j,k)
                           sigma_xy(i,j,k) = sigma_xy(i,vf%cfg%jmax+j,k)
                           sigma_yx(i,j,k) = sigma_yx(i,vf%cfg%jmax+j,k)
                           sigma_yy(i,j,k) = sigma_yy(i,vf%cfg%jmax+j,k)
                        enddo
                     enddo
                  enddo
                  ...
                  do k = vf%cfg%kmino_,vf%cfg%kmaxo_
                     do j = vf%cfg%jmino_,vf%cfg%jmin-1
                        do i = vf%cfg%imino_,vf%cfg%imaxo_
                           sigma_xx(i,vf%cfg%jmax_-j+1,k) = sigma_xx(i,vf%cfg%jmin-j,k)
                           sigma_xy(i,vf%cfg%jmax_-j+1,k) = sigma_xy(i,vf%cfg%jmin-j,k)
                           sigma_yx(i,vf%cfg%jmax_-j+1,k) = sigma_yx(i,vf%cfg%jmin-j,k)
                           sigma_yy(i,vf%cfg%jmax_-j+1,k) = sigma_yy(i,vf%cfg%jmin-j,k)
                        enddo
                     enddo
                  enddo

               end block Stress_Fully_Periodic
      END SELECT
