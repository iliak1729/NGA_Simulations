...
module simulation
   ...
   real(WP) :: radius,radius2,Ycent,Vrise,wavelength,amplitude,phase 
...
contains
   ...
   function levelset_perturbed_cylinder(xyz,t) result(G)
      implicit none 
      real(WP) :: x,y,z,R 
      real(WP), dimension(3), intent(in) :: xyz 
      real(WP), intent(in) :: t
      ...
      R = radius + amplitude*sin(2.0_WP*PI*x/wavelength + phase)
      G = R - sqrt(y*y + z*z)
   end function levelset_perturbed_cylinder
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         ! Initialize a bubble
         call param_read('Tube Radius',radius)
         call param_read('Wave Amplitude',amplitude)
         call param_read('Wavelenth',wavelength)
         ...
         do k=vf%cfg%kmino_,vf%cfg%kmaxo_
            do j=vf%cfg%jmino_,vf%cfg%jmaxo_
               do i=vf%cfg%imino_,vf%cfg%imaxo_
                  ...
                  call cube_refine_vol(cube_vertex,vol,area,v_cent,a_cent,levelset_perturbed_cylinder,0.0_WP,amr_ref_lvl)
