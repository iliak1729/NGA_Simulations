...
module simulation
   ...
   real(WP), dimension(3) :: center,center2,gravity,vel
...
contains
   ...
   function levelset_front(xyz,t) result(G) 
      use param, only: param_read
      implicit none
      real(WP), dimension(3),intent(in) :: xyz
      real(WP), dimension(3) :: dr
   ...
   subroutine simulation_init
      ...
      create_and_initialize_flow_solver: block
         ...
         ! Zero initial field
         call param_read('Bubble velocity',vel)
      ...
      create_and_initialize_temperature_solver : block 
         ...
         call param_read('Liquid Temperature',Ttop)
         call param_read('Gas Temperature',Tbot)
         ! Initial Condition
         do k=vf%cfg%kmino_,vf%cfg%kmaxo_
            do j=vf%cfg%jmino_,vf%cfg%jmaxo_
               do i=vf%cfg%imino_,vf%cfg%imaxo_

                  ts%T(i,j,k) = vf%VF(i,j,k) * Ttop + (1.0_WP - vf%VF(i,j,k)) * Tbot 
               enddo
            enddo
         enddo
